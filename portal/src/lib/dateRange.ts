// Resolves an Agency Overview / marketer preset or custom date range to a
// concrete [since, until] pair, mirroring Meta Business Manager's picker.
// Always excludes today/future, except for the "today" preset itself (which
// the rest of the app otherwise avoids — the overview surfaces it anyway
// since BM's own picker offers it, matching admin expectations more than
// the stricter client-dashboard convention).
//
// Moved here from app/(admin)/admin/overview/dateRange.ts so the marketer
// module can share it; that path re-exports this file.
...
/** Whole days between two ISO dates (until - since). */
export function daysBetweenISO(since: string, until: string): number {
...
export function resolveDateRange(
  ...
  defaultPreset: string = 'this_week'
): ResolvedRange {
  // Default view is This Week on the Agency Overview — matches the default
  // grouping (Marketing Type) as the daily check-in shape most admins land
  // on. The marketer module passes its own default ('30'). Still fully
  // overridable via ?preset=.
  const preset = searchParams.preset || defaultPreset;
