...
// DB-backed mirror of /api/meta/ads — see db/campaigns/route.ts for the
// shared rationale.
export async function GET(req: NextRequest) {
  try {
    ...
    const rows = await query<{ entity_id: string; name: string; effective_status: string; campaign_name: string }>(
      `SELECT entity_id, name, effective_status, campaign_name FROM meta_entities WHERE account_id = $1 AND level = 'ad'`,
      [accountId]
    );
